...
async function mostrarDetalles(serieId){
    try {
        const [serieResponse, actoresResponse] = await Promise.all([fetch(`${API_URL}/series/${serieId}`), fetch(`${API_URL}/series/${serieId}/actores`)]);
        ...
        console.error('Error: ', error);
        modalBody.innerHTML = '<p style="color: #e50914;">Error al cargar los detalles.</p>';
        modal.style.display = 'block';
    }
}
